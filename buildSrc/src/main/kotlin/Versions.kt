import kotlin.String

/**
 * Find which updates are available by running
 *     `$ ./gradlew syncLibs`
 * This will only update the comments.
 *
 * YOU are responsible for updating manually the dependency version. */
object Versions {
    const val jackson_module_kotlin: String = "2.9.7" 

    const val handlebars: String = "4.1.0" //available: "4.1.2" 

    const val com_github_kt3k_coveralls_gradle_plugin: String = "2.8.2" 

    const val gson: String = "2.8.5" 

    const val com_jfrog_bintray_gradle_plugin: String = "1.8.4" 

    const val hamkrest: String = "1.6.0.0" 

    const val com_squareup_moshi: String = "1.7.0" 

    const val okhttp: String = "3.11.0" 

    const val rerunner_jupiter: String = "1.1.1" 

    const val io_github_resilience4j: String = "0.13.1" 

    const val micrometer_core: String = "1.0.7" //available: "1.1.0" 

    const val netty_codec_http2: String = "4.1.23.Final" //available: "4.1.31.Final" 

    const val pebble: String = "3.0.5" 

    const val io_undertow: String = "2.0.14.Final" //available: "2.0.15.Final" 

    const val javax_servlet_api: String = "4.0.1" 

    const val jmfayard_github_io_gradle_kotlin_dsl_libs_gradle_plugin: String =
            "0.2.6" // exceed the version found: 0.2.5

    const val nebula_provided_base_gradle_plugin: String = "3.0.3" //available: "4.0.1" 

    const val net_saliman_cobertura_gradle_plugin: String = "2.5.4" 

    const val gradle_cobertura_plugin: String = "2.5.4" 

    const val argo: String = "5.4" 

    const val cobertura: String = "2.1.1" 

    const val commons_pool2: String = "2.6.0" 

    const val httpasyncclient: String = "4.1.4" 

    const val httpclient: String = "4.5.6" 

    const val httpcore: String = "4.4.10" 

    const val http2_server: String = "9.4.12.v20180830" 

    const val javax_websocket_server_impl: String = "9.4.12.v20180830" 

    const val org_eclipse_jetty: String = "9.4.12.v20180830" 

    const val java_websocket: String = "1.3.9" 

    const val dokka_gradle_plugin: String = "0.9.17" 

    const val org_jetbrains_kotlin: String = "1.2.71" //available: "1.3.0" 

    const val json: String = "20180813" 

    const val jsoup: String = "1.11.3" 

    const val org_junit_jupiter: String = "5.3.1" 

    const val coveralls_gradle_plugin: String = "2.8.2" 

    const val alpn_boot: String = "8.1.13.v20181017" 

    const val selenium_api: String = "3.14.0" 

    const val thymeleaf: String = "3.0.10.RELEASE" //available: "3.0.11.RELEASE" 

    /**
     *
     *   To update Gradle, edit the wrapper file at path:
     *      ./gradle/wrapper/gradle-wrapper.properties
     */
    object Gradle {
        const val runningVersion: String = "4.10.2"

        const val currentVersion: String = "4.10.2"

        const val nightlyVersion: String = "5.1-20181030000041+0000"

        const val releaseCandidate: String = ""
    }
}
